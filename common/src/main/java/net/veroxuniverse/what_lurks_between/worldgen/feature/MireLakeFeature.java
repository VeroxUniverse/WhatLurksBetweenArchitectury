package net.veroxuniverse.what_lurks_between.worldgen.feature;
...
public class MireLakeFeature extends Feature<LakeFeature.Configuration> {

    private static final BlockState AIR = Blocks.CAVE_AIR.defaultBlockState();

    public MireLakeFeature(Codec<LakeFeature.Configuration> codec) {
        super(codec);
    }
            ...
            for (t = 0; t < 16; ++t) {
                for (u = 0; u < 16; ++u) {
                    for (int w = 0; w < 8; ++w) {
                        ...
                        if (bl2 && (w < 4 || randomSource.nextInt(2) != 0)) {
                            BlockState blockState4 = worldGenLevel.getBlockState(blockPos.offset(t, w, u));
                            if (blockState4.isSolid() && !blockState4.is(BlockTags.LAVA_POOL_STONE_CANNOT_REPLACE)) {
                                BlockPos blockPos3 = blockPos.offset(t, w, u);
